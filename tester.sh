...
run_test_case () {
    ...
    # 3: (optional) timeout delay (default is 2 sec and only if strace is false)
    local cmdline="${1}"
    local strace=false
    local delay=2
    ...
    if [[ "${strace}" ==  true ]]; then
        ...
        timeout "${delay}" bash -c \
            "echo -e \"${cmdline}\" | ${SSHELL_EXEC}" >${outfile} 2>${errfile}
    fi
    ...
    [[ ${RET} -eq 127 ]] && warning "Something is wrong (the executable might not exists)"
